...
class ReliabilityGateway:
    ...
    def complete(self, prompt: str) -> GatewayResponse:
        ...
        for index, provider in enumerate(self.providers):
            try:
                response = self.breakers[provider.name].call(provider.complete, prompt)
            except (ProviderError, CircuitOpenError) as error:
                last_error = f"{provider.name}: {error}"
                continue
            if self.cache is not None:
                self.cache.set(prompt, response.text, {"provider": provider.name})
            return GatewayResponse(response.text, "primary" if index == 0 else "fallback",
                                   provider.name, False, response.latency_ms, response.estimated_cost)

        return GatewayResponse("The service is temporarily degraded. Please try again soon.",
                               "static_fallback", None, False, 0.0, 0.0, last_error)
