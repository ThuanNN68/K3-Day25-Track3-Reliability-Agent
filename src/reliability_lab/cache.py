...
class ResponseCache:
    ...
    def get(self, query: str) -> tuple[str | None, float]:
        ...
        if best_entry is not None and best_score >= self.similarity_threshold:
            if _looks_like_false_hit(query, best_entry.key):
                self.false_hit_log.append({"query": query, "cached_key": best_entry.key,
                                           "score": best_score, "reason": "date_or_number_mismatch"})
    ...
    def similarity(a: str, b: str) -> float:
        ...
        def tokens(text: str) -> list[str]:
            ...
            ngrams = [word[index:index + 3] for word in words for index in range(max(0, len(word) - 2))]
            return words + ngrams

        a_counts, b_counts = Counter(tokens(a)), Counter(tokens(b))
        if not a_counts or not b_counts:
            return 0.0
        dot = sum(count * b_counts[token] for token, count in a_counts.items())
...
class SharedRedisCache:
    ...
    def get(self, query: str) -> tuple[str | None, float]:
        ...
        for key in self._redis.scan_iter(f"{self.prefix}*"):
            ...
            if score > best_score:
                best_key, best_query, best_score = key, cached_query, score
        if best_key is not None and best_query is not None and best_score >= self.similarity_threshold:
            if _looks_like_false_hit(query, best_query):
                self.false_hit_log.append({"query": query, "cached_key": best_query,
                                           "score": best_score, "reason": "date_or_number_mismatch"})
                return None, best_score
